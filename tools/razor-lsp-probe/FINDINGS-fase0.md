...
## Fase A — spike de compat do Shiki (colorização) — ✅ PASSA (headless)
A 3ª queixa original ("colorizando corretamente") é a Fase A. O plano exige um **spike de compat ANTES** de trocar o tokenizer de produção. Feito em `spike-shiki.mjs` (`node tools/razor-lsp-probe/spike-shiki.mjs`; requer `npm i shiki @shikijs/monaco`):

- **Resultado: PASSA.** `createHighlighter({themes:["dark-plus","light-plus"], langs:["razor"]})` carrega e tokeniza um `.cshtml` real no Node (WASM Oniguruma) sem erro. As linguagens embutidas **resolvem todas**: `keyword.control.cshtml.transition`/`text.aspnetcorerazor` (razor), `source.cs`/`entity.name.type.cs` (C#), `entity.name.tag.html`/`meta.tag…html` (HTML), `source.css` (CSS). Tema aplica 8 cores distintas.
- **Gotchas de integração descobertos (insumo do A.2):**
  1. **Mismatch de id:** o id do Shiki é `razor`; os ids Monaco aqui são `aspnetcorerazor`/`cshtml`. O `shikiToMonaco` casa por id, então não cobre nossos arquivos sem mapeamento.
  2. **`shikiToMonaco` é takeover GLOBAL:** ele faz monkeypatch de `monaco.editor.setTheme`, redefine temas e registra tokenizer p/ **toda** linguagem carregada (csharp/html/css/js…). Aplicá-lo global trocaria o tema do app inteiro e brigaria com o **layering dos semantic tokens do C#**. → A produção (A.2) deve ser **escopada só aos ids razor** (`aspnetcorerazor`/`cshtml`), sem o monkeypatch global de tema, atrás de flag.
- **Estado:** A.1 (compat) provado/validado. **A.2 (swap de produção escopado + flag)** é o próximo incremento; a **correção de cor e o layering com semantic tokens são inerentemente visuais** → precisa de validação no app (mesmo gate de aceite do brick 6). Não fazer o swap global às cegas.
