...
async fn stream_turn(session: &mut ClaudeSession, job: &PromptJob) -> Result<TurnEnd, TurnFailure> {
...
async fn run_job(
    ...
    match outcome {
        ...
        Err(TurnFailure::ResumeLost) => {
            ...
            let _ = job.on_event.send(AcpEvent::Status {
                message: "Não foi possível retomar a sessão anterior; recriando a conversa…".into(),
            });
            *current =
                Some(spawn_session(root, &job.conversation_id, job.mode, &job.model, None).await?);
