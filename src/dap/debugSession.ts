/**
 * .NET debug session manager (roadmap csharp-ide-parity, Fase B).
 *
 * One session at a time. Owns the DAP dance against netcoredbg:
 ...
 *
 * Breakpoints live HERE (per absolute file path, 1-based lines) so they exist
 * before/after sessions; the editor renders them and calls `toggleBreakpoint`.
 * React subscribes via `subscribe`/`getState` (useSyncExternalStore-friendly).
 */
import {
  dapEnsureNetcoredbg,
...
export interface DebugState {
  ...
  /** Scopes+variables of the selected (top) frame. */
  scopes: ScopeView[];
  /** Absolute path → sorted 1-based lines. */
...
class DebugSessionManager {
  ...
  private setState(patch: Partial<DebugState>): void {
    this.state = {
      ...
      breakpoints: Object.fromEntries(
        [...this.breakpoints.entries()].map(([p, s]) => [p, [...s].sort((a, b) => a - b)])
      ),
    };
  ...
  toggleBreakpoint(path: string, line: number): void {
    let set = this.breakpoints.get(path);
    if (!set) {
      set = new Set();
      this.breakpoints.set(path, set);
    }
    if (!set.delete(line)) set.add(line);
    if (set.size === 0) this.breakpoints.delete(path);
    this.setState({});
    // Live session: push the file's new set immediately.
    if (this.client) void this.pushBreakpoints(path);
  }

  breakpointsFor(path: string): number[] {
    return [...(this.breakpoints.get(path) ?? [])].sort((a, b) => a - b);
  }
  ...
  private async pushBreakpoints(path: string): Promise<void> {
    ...
    try {
      await this.client.request("setBreakpoints", {
        source: { path },
        breakpoints: this.breakpointsFor(path).map((line) => ({ line })),
      });
    } catch (err) {
  ...
  /** Build the csproj and launch its DLL under the debugger. */
  async launchProject(csprojPath: string, cwd: string): Promise<void> {
    await this.start(async (client) => {
      ...
      this.appendOutput(`[launch] dotnet ${dll}`);
  ...
  private async start(requestSession: (client: DapClient) => Promise<void>): Promise<void> {
    ...
    this.setState({ status: "starting", error: undefined, output: [], frames: [], scopes: [], stoppedAt: undefined });
    try {
  ...
  private wireEvents(client: DapClient): void {
    ...
    client.on("continued", () => {
      this.setState({ status: "running", frames: [], scopes: [], stoppedAt: undefined });
    });
    const ended = (): void => {
      void this.stop();
    };
    ...
    client.onceClosed(() => {
      if (this.client === client) {
        ...
        if (this.state.status !== "idle" && this.state.status !== "error") {
          this.setState({ status: "idle", frames: [], scopes: [], stoppedAt: undefined });
        }
      }
    });
  }

  /** On stop: fetch top frames + the top frame's scopes/variables. */
    ...
    try {
      ...
      if (top) {
        ...
        for (const s of sc.scopes ?? []) {
          ...
          scopes.push({
            name: s.name,
            variables: (vars.variables ?? []).map((v) => ({ name: v.name, value: v.value, type: v.type })),
          });
        }
      }
      this.setState({
